...
app.get("/movies",function(req,res){
        ...
        .then((result) => {
            const movies = [result];
            res.render("movies", {movie: result[2].title, description: result[2].plot, image: result[2].runTime} );
        })
        .catch((err) => {
            console.log(err);
        })
 ...
 app.get("/userin",function(req,res){
    ...
    .then((result) => {
        for(i = 1; i < result.length; i++){
            res.render("userin.ejs", {title: result[i].title, genre: result[i].genre ,length: result.lenth, img: result[i].img, key: result[i].keyword});
        }
